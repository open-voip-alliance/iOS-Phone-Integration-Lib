...
@interface MiddlewareRequestOperationManager () <MiddlewareDelegate> 
...
- (void)updateDeviceRecordWithAPNSToken:(NSString *)apnsToken sipAccount:(NSString *)sipAccount withCompletion:(void (^)(NSError *error))completion {
    ...
    if (sipAccount.length == 0) {
        NSLog(@"SIP Account was not set, could not update middleware.");
    }
    if (apnsToken.length == 0) {
        NSLog(@"APNS Token was not provided, could not update middleware.");
    }
...
//    if ([VialerLogger remoteLoggingEnabled]) { //wip
