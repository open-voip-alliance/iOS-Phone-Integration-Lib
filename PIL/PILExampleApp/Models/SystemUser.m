    ...
    [self updateUseEncryptionWithCompletion:^(BOOL success, NSError *error) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
            if (self.loggedIn && self.sipEnabled) {
                NSLog(@"post from setUseTLS");
                [[NSNotificationCenter defaultCenter] postNotificationName:SystemUserEncryptionUsageChangedNotification object:self];
            } else {
                NSLog(@"Nothing to do here sip has not been enabled");
            }
        });
    }];
}
...
- (void)setUseStunServers:(BOOL)useStunServers {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        if (self.loggedIn && self.sipEnabled) {
            NSLog(@"Post from setUseStunServers");
...
- (void)fetchUserProfile {
    if (self.loggingOut) {
        NSLog(@"Already logging out, no need to fetch user profile");
        return;
    }
...
- (NSString *)parseStringForClientID:(NSString *)givenString {
    ...
    if (!givenString || [givenString isEqualToString:@""]) {
        ...
        NSLog(@"Unable to find a Client ID in string:%@", givenString);
        return nil;
    }
}
