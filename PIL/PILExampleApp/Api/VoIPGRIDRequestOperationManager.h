 ...
 */
- (instancetype _Nonnull)initWithDefaultBaseURL;
 ...
 */
- (instancetype _Nonnull)initWithDefaultBaseURLandRequestOperationTimeoutInterval:(NSTimeInterval)requestTimeoutInterval;
 ...
 *
 *  @param URLString        The url that will be requested.
 ...
 *  @param completion   A block that will be called after the push attempt. It will return the response data if any or an error if any.
 */
- (void)pushUseOpus:(BOOL)enable withCompletion:(void(^_Nullable)(BOOL success, NSError * _Nullable error))completion;
